...
def contact(request):
    if request.method == 'POST': 
        ...
        # check if the user already made an inquary
        if request.user.is_authenticated:
            user_id = request.user.id 
            has_contacted = Contact.objects.all().filter(listing_id=listing_id, user_id=user_id)
            if has_contacted:
                messages.error(request, "You have already made an inquary for this listing.")
        ...
        contact.save()
        send_mail(
            "BTRE Inquary submitted: " + listing, 
            "Message from the user: " + message, 
            "yair.churi@gmail.com",
            [realtor_email, "yair.churi@gmail.com"], 
            fail_silently=False,
            )
